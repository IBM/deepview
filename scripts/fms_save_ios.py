...
if args.compile:
        ...
        def print_model_hierarchy():
            ...
            def post_hook_fn(module, input, output):
                ...
                if hasattr(module, '_debug_input'):
                    save_path = save_ios(layer_name, module._debug_input, output)
                    layer_stack.append((layer_name,save_path))
                    print(f"Saved I/O to {save_path}")
                    # Clean up
    ...
    # save the models layers stack
    save_layer_stack(layer_stack)

    # Rashed 
    print("End: fms_layers saved")
    exit(1)
