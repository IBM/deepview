...
def hook_func_fwd(module, grad_input, grad_output):
    name = module_name_dict[module]
    print("FORWARD HOOK INPUT: ", name, module, "\n", grad_input)
    print("FORWARD HOOK OUTPUT: ", name, module, "\n", grad_output)

def hook_func_bwd(module, grad_input, grad_output):
    name = module_name_dict[module]
    print("BACKWARD HOOK INPUT: ", name, module, "\n", grad_input)
    print("BACKWARD HOOK OUTPUT: ", name, module, "\n", grad_output)
def register_hooks(model, is_forward=True):
    hooks = []
    for name, m in model.named_modules():
        # skip non-primitive modules
        if len(list(m.modules())) > 1:
            continue
        module_name_dict[m] = name
        if is_forward:
            hook = m.register_forward_hook(hook_func_fwd)
        else:
            hook = m.register_backward_hook(hook_func_bwd)
        hooks.append(hook)
    return hooks
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="Whether to compile loss fuunction (-1: not comple loss, 0: follow compile option(default), 1: compile loss, 2:compile loss only ).",
)
parser.add_argument(
    "--head_only",
    action="store_true",
...
parser.add_argument(
    "--forward_hook",
    action="store_true",
    help="Whether to trace input/output tensors for forward pass.",
)
parser.add_argument(
    "--backward_hook",
    action="store_true",
...
def get_loss_fn():
    ...
    def loss_fn(output, target):
        ...
        #logsoftmax = log_softmax(output)
        #loss = nll_loss(logsoftmax, target)
        #print("XXX LOSS_FN(output={}, target={}): logsoftmax={}, loss={}".format(output, target, logsoftmax, loss))
        #return loss
...
def main():
    ...
    compile_loss_only = False
    if args.compile_loss == -1:
      compile_loss = False
    elif args.compile_loss == 1:
      compile_loss = True
    elif args.compile_loss == 2:
      compile_loss = False
      compile_loss_only = True
