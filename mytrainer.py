...
def __one_step(
    ...
    **kwargs,
    ...
    def loss_fwd_compile_loss(input, model, label, **kwargs):
        output = model(input, attn_algorithm="math", **kwargs)
        loss = torch.compile(loss_fn, backend="sendnn")
        loss = loss(output, label)
        return loss

    if compile_loss:
        loss_fwd = torch.compile(loss_fwd, backend=compile_backend)
    if compile_loss_only:
        loss_fwd = loss_fwd_compile_loss
